...
@section('content')
    ...
    <style>
        ...
        * {
        ...
        .glow-yellow {
            box-shadow: 0 0 40px rgba(255, 193, 7, 0.18);
        }
        ...
        .card-stack:hover .card-stack-layer:nth-child(1) {
            transform: rotate(-10deg) translateX(18px);
        }

        .card-stack:hover .card-stack-layer:nth-child(2) {
            transform: rotate(-5deg) translateX(10px);
        }

        .card-stack:hover .card-stack-main {
            transform: translateY(-6px);
            box-shadow: 0 24px 80px rgba(255, 193, 7, 0.25);
        }
        ...
        .stat-card.dark .stat-link {
            color: var(--yellow);
        }

        .stat-card.dark .stat-link-icon {
            background: var(--yellow);
            color: #000;
        }

        .stat-card.light .stat-link {
            color: #000;
        }

        .stat-card.light .stat-link-icon {
            background: #000;
            color: var(--yellow);
        }
        ...
        .twinkle-3 {
            animation: twinkle 2.3s ease-in-out infinite 1s;
        }

        .twinkle-4 {
            animation: twinkle 3.1s ease-in-out infinite 1.5s;
        }
        ...
        .twinkle-6 {
            animation: twinkle 2.6s ease-in-out infinite 0.3s;
        }
        ...
        @media (max-width: 1024px) {
            ...
            .program-grid .card-stack {
                order: 2;
            }

            .program-grid .program-text {
                order: 1;
            }
    ...
    <div class="page-wrapper">
        ...
        <svg class="bg-lines" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
            <path d="M 80 150 Q 350 280 250 550 T 700 950" fill="transparent" stroke="#FFC107" stroke-width="1.5"
                stroke-dasharray="8 14" />
            <path d="M 900 100 Q 1100 400 1000 700 T 1200 1400" fill="transparent" stroke="#FFC107" stroke-width="1.5"
                stroke-dasharray="8 14" />
            <path d="M 400 1200 Q 700 1500 600 1900" fill="transparent" stroke="#FFC107" stroke-width="1.5"
                stroke-dasharray="8 14" />
        </svg>
    ...
    <script>
        // ==========================================
        // 1. EFEK INTERAKTIF: PARTICLE NETWORK (Tetap Ada)
        // ==========================================
        ...
        class ParticleNode {
            ...
            update() {
                ...
                let dx = mouseObj.x - this.x;
                let dy = mouseObj.y - this.y;
        ...
        function initParticles() {
            ...
            for (let i = 0; i < numberOfParticles; i++) {
                ...
                particlesArray.push(new ParticleNode(x, y, directionX, directionY, size, 'rgba(255, 193, 7, 0.4)'));
            }
        }
            ...
            for (let a = 0; a < particlesArray.length; a++) {
                for (let b = a; b < particlesArray.length; b++) {
                    ...
                    if (distance < (canvas.width / 12) * (canvas.height / 12)) {
                        ...
                        ctx.strokeStyle = 'rgba(255, 193, 7, ' + opacityValue / 4 + ')';
                        ctx.lineWidth = 0.5;
                        ctx.beginPath();
                ...
                if (mouseObj.x) {
                    let dx = mouseObj.x - particlesArray[a].x;
                    let dy = mouseObj.y - particlesArray[a].y;
                    let distanceMouse = dx * dx + dy * dy;
                    if (distanceMouse < mouseObj.radius * mouseObj.radius) {
                        opacityValue = 1 - (distanceMouse / (mouseObj.radius * mouseObj.radius));
                        ctx.strokeStyle = 'rgba(255, 193, 7, ' + opacityValue / 1.5 + ')';
                        ctx.lineWidth = 1;
                        ctx.beginPath();
                        ctx.moveTo(particlesArray[a].x, particlesArray[a].y);
                        ctx.lineTo(mouseObj.x, mouseObj.y);
                        ctx.stroke();
                    }
                }
            }
        ...
        // ==========================================
        // 2. MOUSE TRACKING (Untuk Partikel)
        // ==========================================
        document.addEventListener('mousemove', e => {
            let zoom = window.innerWidth >= 1024 ? 1.25 : 1;
            mouseObj.x = e.clientX / zoom;
            mouseObj.y = e.clientY / zoom;
        });

        document.addEventListener('mouseout', () => {
            mouseObj.x = undefined;
            mouseObj.y = undefined;
        });

        // ==========================================
        // 3. SCROLL REVEAL & LAINNYA
        // ==========================================
