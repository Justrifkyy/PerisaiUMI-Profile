...
@section('title', 'Add Member - ' . $department->name)
...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-gray-800 rounded-lg shadow-xl p-8">
            <h2 class="text-2xl font-bold text-white mb-6">Add New Member to {{ $department->name }}</h2>

            <form action="{{ route('admin.departments.members.store', $department) }}" method="POST"
                ...
                <div>
                    ...
                    <input type="text" name="position" id="position" value="{{ old('position') }}" required
                        placeholder="e.g., King of Ristek Sispro"
                        class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-yellow-400">
                    @error('position')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="level" id="level" value="{{ old('level', 0) }}"
                            class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-yellow-400">
                        <p class="mt-1 text-xs text-gray-400">For hierarchical ordering</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="order" id="order" value="{{ old('order', 0) }}"
                            class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-yellow-400">
                        <p class="mt-1 text-xs text-gray-400">Display order within same level</p>
                    </div>
                </div>
